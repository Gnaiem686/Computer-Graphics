...
#include <iostream>
	...
	void Scene::ShapeTransformation(int shapeindex, int type, float amt)
	{
		if(glm::abs(amt)>1e-5)
		{
			switch (type)
			{
			case xTranslate:
				shapes[shapeindex]->MyTranslate(glm::vec3(amt,0,0),0);
			break;
			case yTranslate:
				shapes[shapeindex]->MyTranslate(glm::vec3(0,amt,0),0);
			break;
			case zTranslate:
				shapes[shapeindex]->MyTranslate(glm::vec3(0,0,amt),0);
			break;
			case xRotate:
				shapes[shapeindex]->MyRotate(amt,glm::vec3(1,0,0),0);
			break;
			case yRotate:
				shapes[shapeindex]->MyRotate(amt,glm::vec3(0,1,0),0);
			break;
			case zRotate:
				shapes[shapeindex]->MyRotate(amt,glm::vec3(0,0,1),0);
			break;
			default:
				break;
			}
